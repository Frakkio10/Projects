...
class DREFRAP(Profile):
    ...
    def __init__(self, shot, flag, twindow=None, verbose=False):
    ...
    def get_ne(self, twindow=None,  averaged=False, Rshift=0, verbose=True):
        ...
        if twindow is not None:
           self = DREFRAP(shot=self.shot, flag = self.flag, twindow=twindow)
        ...
        if not averaged:
            ...
            #rho_psi = np.sort(np.append(rho_psi, 1.3))
            #ne = np.sort(np.append(ne, 0))[::-1]
...
if __name__ == '__main__':
    ...
    # %matplotlib widget
    nprof = DREFRAP(60270, flag = 1, twindow=[5,6], verbose = True)
