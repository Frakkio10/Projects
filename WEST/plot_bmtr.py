...
#fro
from DBS.beamtracing.custom.custom_beamtracing import custom_beam3d
from DBS.beamtracing.custom.reflectometers_v1 import DREFRAP
#%%
machine, shot, twindow, xmode, channelval = 'west', 58333, [23.8, 24.6], 1, 2
flag = 0
isweep = 24
output_24, interface_24 = custom_beam3d(machine, shot, xmode = xmode, isweep = isweep, channelval = channelval, ne_max = 4.5, drho = -0.035)
#%%
machine, shot, twindow, modex, channelval = 'west', 58333, [14.2, 14.8], 1, 2
flag = 0
isweep = 8
output_8, interface_8 = custom_beam3d(machine, shot, xmode = xmode, isweep = isweep, channelval = channelval, ne_max = 4.5, drho = -0.035)
...
for ifreq, freq in enumerate(output_24.freqGHz[0:20:2]):
    beam = output_24.beam[ifreq]
    dif = output_24.dif[ifreq]
    beami = output_24.beami[ifreq]
    beami.diagnm = '' # 'tcvvx' or 'difdop', but not sure this is actually needed
    integopt = output_24.integopt[ifreq]
    # fig, ax = plt.subplots()
