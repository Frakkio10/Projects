...
class TIMETRACE():
    ...
    def DIFDOP(self, tstart, tend, isweep = None):
        ...
        difdop = dict({'header':header_txt, 'shot':self.shot, 'time':time, 'anglepol_ver':anglepol_ver, 'anglepol_inc':anglepol_inc, 'window':[tstart, tend], 'parameters':params, 't_DIFDOP':t_DIFDOP, 't_acq':t_acq})
        self.difdop = difdop 
        return difdop
    ...
    def plot_tt(self, isweep = None, colors = None, ax = None, save = False):
        ...
        #start plotting
        ax.plot(difdop['time'], difdop['anglepol_inc'], 'silver', lw = 1, label = 'inclinometer [°]')
        ax.plot(difdop['time'], difdop['anglepol_ver'], 'skyblue', lw = 1, label = 'verin [°]')
